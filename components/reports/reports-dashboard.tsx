...
export function BusinessDashboard({ records, pets }: Props) {
  ...
  /* FILTROS - Con protección para datos incompletos */
  const filteredRecords = useMemo(() => {
    let data = records

    if (startDate && endDate) {
      data = data.filter(record => {
        // Solo filtramos si existe la fecha, si no, se ignora
        if (!record.entryDate) return false
        return isWithinInterval(parseISO(record.entryDate), {
  ...
  const servicesChart: ChartData[] = useMemo(() => {
    ...
    filteredRecords.forEach(r => {
      const name = r.serviceName || "Otros"
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="bg-white p-6 rounded-xl shadow grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <select value={heightRange} onChange={e => setHeightRange(e.target.value)} className="border p-2 rounded-lg">
        ...
        <button onClick={() => { setStartDate(""); setEndDate(""); setHeightRange(""); }} className="bg-gray-200 rounded-lg px-4 py-2">
          Limpiar
        </button>
      </div>
      ...
      {/* PREDICCION - Aquí es donde se usa la variable para que no dé warning */}
      <div className="bg-white p-6 rounded-xl shadow text-center">
        <p className="text-sm text-gray-500">Predicción próximo mes</p>
      ...
      <div className="sticky bottom-0 bg-white p-4 flex flex-col sm:flex-row gap-4 justify-end border-t shadow-lg">
        <button onClick={exportExcel} className="bg-amber-500 text-white px-6 py-2 rounded-lg">Descargar Excel</button>
        <button onClick={exportPDF} className="bg-gray-800 text-white px-6 py-2 rounded-lg">Descargar PDF</button>
      </div>
    </div>
  )
}
